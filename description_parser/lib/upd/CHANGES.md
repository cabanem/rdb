...
### 2. Check rows: the spec disagrees with a filled cell (`Extractor.js` + `DescriptionParser.js`)
...
- Note starts with `Check:` and is phrased as a question ("The spec says Optional. Untick Required?"), then says what happens if it is left as is. The severity (error/warn) is kept in the payload, not shown.
- Proposed value = the fix, or `(decide by hand — no value to apply)`.
...
| Cell                     | Spec                                   | Row                                                   |
|--------------------------|----------------------------------------|-------------------------------------------------------|
| Required = TRUE          | `Optional`                             | Check, fix = FALSE                                     |
| Required = TRUE          | `Optional/Required` + a readable rule  | Check, fix = FALSE, "untick and accept the proposed rule instead?" |
| Required = TRUE          | `Optional/Required`, tenant-level condition | Check, fix = FALSE, quotes the condition, "required for this project?" |
| Required blank           | `Optional/Required`, no readable rule  | Check, no fix, quotes the condition                    |
| Data type = date         | `Character (4)`, `Example: 0214`       | Check, fix = string                                    |
| Data type = string       | `Decimal` / `Date`                     | Check, fix = float (2) / date                          |
...
| Check rows (spec disagrees with a filled cell) | 0 |    33 |

Of the 33 check rows, 26 are "Required is ticked but the spec makes it conditional".
