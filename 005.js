 ...
 *
 * Outcomes on the wire (API-00):                                                         // ***UPDATED***
 *   ok:false                 -> endpoint rejected; thrown as stage 'endpoint-rejected'.
 *   ok:true, mode:'unchanged' -> config_fingerprint matched the published version; Workato cut no new
 *                               version and rebuilt nothing. Returned as a success Result with
 *                               data.mode = 'unchanged' and NO validation verdict (there is none).
 *   ok:true (any other mode) -> a new version was provisioned; verdict + form channel are resolved and
 *                               written to _validation_results as before.
 *
 * Public:
...
Provision.run = function (ss, options) {
  ...
  try {
    ...
    // 8. No-op: the fingerprint matched the published version, so Workato left it in place.          // ***UPDATED***
    //    Return before the verdict/report step - there is no verdict on this response, and running
    //    ValidationReport.write here would only produce a misleading 'no-verdict' warning.
    if (response.parsed && response.parsed.mode === 'unchanged') {
      var liveVersion = response.parsed.version_number
        ? 'v' + response.parsed.version_number
        : 'the current version';
      log('SUCCESS', 'Provision complete (no-op): configuration unchanged; ' + liveVersion + ' remains published.');

      return Result.ok({
        flow: 'provision',
        correlationId: correlationId,
        message: 'Configuration unchanged. ' + liveVersion + ' is already published; nothing was rebuilt.',
        warnings: shareResult.failed.map(function (f) {
          return 'Audit-share failed for ' + f.email + ': ' + f.error;
        }),
        data: {
          mode: 'unchanged',
          versionNumber: response.parsed.version_number || null,
          templateVersionId: response.parsed.template_version_id || '',
          applicationName: pf.applicationName,
          configJsonFileId: configJsonFileId
        }
      });
