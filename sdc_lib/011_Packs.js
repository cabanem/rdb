/**
 * @file 011_Packs.gs
 * Field packs: A reviewed, config-shaped Google Sheet per VMS by record type, applied into an implementation
 *              workbook so the analyst begins from a filled 4_fields sheet, instead of blank per implementation.
 * 
 ...
 * _review:     curator notes; never ingested by the library.
 * 
 * Packs are read-only to analysts. Packs.apply copies rows OUT of the pack into the open workbook.
 * Client-specific changes are made in the client workbook only.
 * 
 * Pipeline (Packs.apply):
 *  Config.build --> Packs.read --> Packs.plan --> write tabs (under document lock) -->
 ...
 *  Packs.read(fileId)                      -> pack object (see Packs.readPack)
 *  Packs.plan(pack, answers, scope, mode)  -> { fields, rules, lookups, stamps, warnings } (no Sheets calls)
 *  Packs.apply(ss, opts)                   -> Result
 *    opts.fileId   pack Sheet ID (required)
 *    opts.answers  { question_id: true|false }                         --default: every quesetion false
 *    opts.scope    [field names to keep] | null                        --default: every field
 *    opts.mode     'standard'|'discovery'                              --default: 'standard' (discovery sets Strict? FALSE on each row)
 *    opts.replace  true to overwrite a 4_fields that already has rows  --default: false, fail w/stage 'guard'
 * 
 * Layout is located by anchor text ('_pk_fields_', '_pk_rules_', '_pk_lookup_table_' or 'Table name'), not by fixed row numbers.
 */
 ...
 * the connector expects one on 4_fields, 5_lookups AND 4_complex_validations. (Since schema 1.7 PrimaryKey.backfill
 * also covers the rules tab via the _pk_rules_ entry; stamping here as well keeps the pack write self-contained.)
 * Text columns are set to plain-text format first, so a code like "1" is not turned into the number 1 by setValues.
 */
